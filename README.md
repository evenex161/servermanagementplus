...
5. **Verify it Yourself**: If you are a server administrator or code reviewer and want to safely test this system, launch a local test server and explicitly opt-in to the Smart Restart feature via the in-game admin menu. You will observe that the mod securely backs up your run scripts to `.bak` files and cleanly injects the `-Dservermanagement.smartstart=true` flag. Nothing is hidden.
