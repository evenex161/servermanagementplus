...
public class MineBayOffer {
    ...
    public CompoundTag toNBT() {
        ...
        for (int i = 0; i < itemOffers.size(); i++) {
            itemOffersTag.put("Item" + i, itemOffers.get(i).saveOptional(net.neoforged.neoforge.server.ServerLifecycleHooks.getCurrentServer().registryAccess()));
        }
    ...
    public static MineBayOffer fromNBT(CompoundTag tag) {
        ...
        for (int i = 0; i < itemCount; i++) {
            offer.itemOffers.add(ItemStack.parseOptional(net.neoforged.neoforge.server.ServerLifecycleHooks.getCurrentServer().registryAccess(), itemOffersTag.getCompound("Item" + i)));
        }
