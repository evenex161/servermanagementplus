...
public final class ModNetworking {
    ...
    public static void registerServerPackets() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ModFileRequestPacket.ID, (server, player, handler, buf, sender) -> { ModFileRequestPacket pkt = new ModFileRequestPacket(buf); server.execute(() -> pkt.handle(player)); });
    ...
    public static void registerClientPackets() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(VersionCheckPacket.ID, (client, handler, buf, sender) -> { VersionCheckPacket pkt = new VersionCheckPacket(buf); client.execute(() -> pkt.handle(null)); });
        ClientPlayNetworking.registerGlobalReceiver(ModFileChunkPacket.ID, (client, handler, buf, sender) -> { ModFileChunkPacket pkt = new ModFileChunkPacket(buf); client.execute(() -> pkt.handle(null)); });
        ClientPlayNetworking.registerGlobalReceiver(ModFileCompletePacket.ID, (client, handler, buf, sender) -> { ModFileCompletePacket pkt = new ModFileCompletePacket(buf); client.execute(() -> pkt.handle(null)); });
