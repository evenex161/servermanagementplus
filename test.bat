@echo off
REM ============================================
REM  ServerManagement+ Debug Test Launcher (mc/1.20.1 branch)
REM  Usage: test.bat [client|server|both|debug]
REM         test.bat [client|server] [forge|fabric]
REM ============================================
REM  Modes:
...
REM    forge    - Forge loader (default)
REM    fabric   - Fabric loader
REM    (NeoForge intentionally omitted on the mc/1.20.1 branch)
REM ============================================
...
if /I not "%LOADER%"=="forge" if /I not "%LOADER%"=="fabric" (
    echo [TEST] Unsupported loader on mc/1.20.1: %LOADER%
    echo [TEST] Supported: forge ^| fabric
    exit /b 1
)
...
echo Usage: test.bat [client^|server^|both^|debug] [forge^|fabric]
exit /b 1
...
REM On Forge / Fabric (mc/1.20.1) no per-loader artifact-lock workaround is required;
REM the buildSrc dir is redirected via BUILDSRC_ALT_BUILD_DIR above.
